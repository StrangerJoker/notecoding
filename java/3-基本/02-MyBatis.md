...
```xml
    <resultMap id="customerMap" type="com.uestc.domain.Customer">
        <!-- 定义列名和属性 -->
        <!-- 主键类型用id 标签-->
        <id column="id" property="cid" />
        <!-- 非主键类型用result标签-->
    ...
    <!-- 测试 数据库名字 和 类属性不同的 的问题 -->
    <!-- 解决办法
        1. 先用 resultMap 定义 不同名字之间的映射关系
        2. 再在 select 标签中用 resultMap 属性， 值为resultMap标签中的id
    -->
    <select id="selectCustomerById" resultMap="customerMap">
        select id, name, email, age from student where id=#{CustomerId}
    </select>
```
...
```xml
    <!-- 测试 数据库名字 和 类属性不同的 的问题 -->
    <!-- 解决办法二   resultType标签使用别名 -->
    <select id="selectCustomerById1" resultType="com.uestc.domain.Customer">
        select id as cid, name as cname, email as cemail, age as cage from student where id=#{CustomerId}
    </select>
```
